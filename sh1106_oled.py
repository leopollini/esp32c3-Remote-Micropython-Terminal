...
class SH1106(framebuf.FrameBuffer):
    def __init__(self, width, height, external_vcc, rotate=0):
        if rotate == 0:
            self.HORIZONTAL_OFFSET = 28
            self.VERTICAL_OFFSET = 12
        if rotate == 90:
            self.HORIZONTAL_OFFSET = 12
            self.VERTICAL_OFFSET = 28
        if rotate == 180:
            self.HORIZONTAL_OFFSET = 28
            self.VERTICAL_OFFSET = 12
        if rotate == 270:
    ...
    def text(self, text, x, y, color=1, auto_show = True):
        x += self.HORIZONTAL_OFFSET
        y += self.VERTICAL_OFFSET
    ...
    def line(self, x0, y0, x1, y1, color):
        ...
        y0 += self.VERTICAL_OFFSET
        x1 += self.HORIZONTAL_OFFSET
        y1 += self.VERTICAL_OFFSET
    ...
    def fill(self, color, auto_show = True):
        super().fill(color)
...
class SH1106_I2C(SH1106):
    def __init__(self, width=128, height=64, i2c=I2C(sda=Pin(5), scl=Pin(6)), res=None, addr=0x3c,
